...
<input>{"task_id":"test.task","status":"success","verification_passed":true,"tests_passing":"5/5","coverage":80,"files_modified":["test.ts"],"duration":"1m","acceptance_criteria_met":"1/1","notes":"Done"}</input>
    ...
    it('should parse implementation learnings', () => {
      const output = `
<tool_call>
<name>report_implementation_result</name>
<input>{"task_id":"auth.signup","status":"success","verification_passed":true,"tests_passing":"8/8","coverage":91,"files_modified":["src/auth/signup.ts"],"duration":"2m","acceptance_criteria_met":"2/2","notes":"Implemented signup flow","learnings":[{"type":"pattern","content":"Zod schemas in src/validators/ are reused by API routes","scope":"global"},{"type":"context","content":"Auth middleware at lib/auth.ts","scope":"directory","directory":"src/api"}]}</input>
</tool_call>`;
      ...
      expect(result.learnings).toEqual([
        {
          type: 'pattern',
          content: 'Zod schemas in src/validators/ are reused by API routes',
          scope: 'global',
        },
        {
          type: 'context',
          content: 'Auth middleware at lib/auth.ts',
          scope: 'directory',
          directory: 'src/api',
        },
      ]);
...
<input>{"task_id":"auth.login","status":"success","verification_passed":true,"attempts":1,"fix_type":"dependency","hypothesis":"Missing bcrypt dependency","solution_applied":"Installed bcrypt@5.1.0","notes":"Successfully installed missing dependency and all tests pass"}</input>
    ...
    it('should parse healing result with heal pattern', () => {
      const output = `
<tool_call>
<name>report_healing_result</name>
<input>{"task_id":"auth.login","status":"success","verification_passed":true,"attempts":1,"fix_type":"dependency","hypothesis":"Prisma client was not generated","solution_applied":"Ran prisma generate","notes":"Known fix resolved the issue","heal_pattern":{"error_signature":"Module not found: @prisma/client","fix_command":"npx prisma generate"}}</input>
</tool_call>`;

      const result = StructuredOutputParser.parseHealingResult(output);

      expect(result.heal_pattern).toEqual({
        error_signature: 'Module not found: @prisma/client',

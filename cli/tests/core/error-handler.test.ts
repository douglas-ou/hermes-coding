...
describe('Errors', () => {
  describe('taskNotFound', () => {
    it('should create task not found error with task ID', () => {
      ...
      expect(error.recoverable).toBe(false);
      expect(error.suggestedAction).toBe('Use "hermes-coding tasks list" to see available tasks');
    });
  });
  ...
  describe('stateNotFound', () => {
    it('should create state not found error', () => {
      ...
      expect(error.message).toBe('No active hermes-coding session');
      expect(error.recoverable).toBe(false);
      expect(error.suggestedAction).toBe('Start a new session with /hermes-coding command');
    });
  });

...
describe('Service Factory', () => {
  ...
  beforeEach(async () => {
    ...
    // Create .hermes-coding structure
    const hermesCodingDir = path.join(testDir, '.hermes-coding');
    const tasksDir = path.join(hermesCodingDir, 'tasks');
    await fs.ensureDir(tasksDir);

    // Create minimal state.json
    await fs.writeJson(path.join(hermesCodingDir, 'state.json'), {
      phase: 'implement',
      currentTask: null,
  ...
  describe('createPromptService', () => {
    it('should create a PromptService instance', () => {
      const promptService = createPromptService();

      expect(promptService).toBeDefined();
      expect(typeof promptService.renderImplementerPrompt).toBe('function');
    });

    it('should create independent prompt service instances', () => {
      const service1 = createPromptService();
      const service2 = createPromptService();
  ...
  describe('workspace directory handling', () => {
    it('should handle workspace directory with spaces in path', async () => {
      ...
      await fs.ensureDir(path.join(spacedDir, '.hermes-coding', 'tasks'));
      await fs.writeJson(path.join(spacedDir, '.hermes-coding', 'state.json'), {
        phase: 'clarify',
        currentTask: null,
        startedAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
      });
      await fs.writeJson(path.join(spacedDir, '.hermes-coding', 'tasks', 'index.json'), {
        version: '1.0.0',
        updatedAt: new Date().toISOString(),
        metadata: {},
        tasks: {},
    ...
    it('should use correct tasks directory path structure', async () => {
      ...
      const taskFilePath = path.join(workspaceDir, '.hermes-coding', 'tasks', 'mymodule', 'mytask.md');

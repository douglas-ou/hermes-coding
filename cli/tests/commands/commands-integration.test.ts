...
describe('Commands Integration Tests', () => {
  ...
  describe('Task Workflow', () => {
    ...
    it('should persist lifecycle timestamps across service save and reload', async () => {
      ...
      const completedTask = await services.taskService.completeTask('auth.login', '25 minutes');
      const reloadedTask = await services.taskService.getTask('auth.login');

      expect(completedTask.completedAt).toBeDefined();
      expect(reloadedTask?.startedAt).toBeDefined();
  ...
  describe('Detection Workflow', () => {
    ...
    it('should detect and save to index with verification', async () => {
      ...
      const indexPath = path.join(workspaceDir, '.hermes-coding', 'tasks', 'index.json');
  ...
  describe('Service Integration', () => {
    ...
    it('should handle embedded healing workflow with errors', async () => {
      // Arrange - Initialize to implement phase (healing is now embedded in implement)
      await services.stateService.initializeState('implement');
      ...
      // Act - Create fix task to address the failure (simulates healer sub-agent output)
      await services.taskService.createTask({
        id: 'broken.task.fix',
        module: 'broken',
        priority: 1,
      ...
      // Assert - Phase is still implement (healing doesn't change phase)
      const finalState = await services.stateService.getState();

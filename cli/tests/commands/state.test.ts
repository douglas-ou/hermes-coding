...
describe('state commands', () => {
  ...
  describe('state get', () => {
    ...
    it('should show no active session when state file does not exist', async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('No active hermes-coding session'));
  ...
  describe('state set', () => {
    ...
    it('should output JSON when --json flag is used', async () => {
      registerStateCommands(program, testDir);
      await program.parseAsync(['node', 'test', 'state', 'set', '--phase', 'deliver', '--json']);

      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining('"phase": "deliver"')
      );
    });

    it('should validate all phases', async () => {
      const validPhases = ['clarify', 'breakdown', 'implement', 'deliver'];
    ...
    it('should handle file system error during set', async () => {
      ...
      const hermesCodingDir = path.join(testDir, '.hermes-coding');
      fs.ensureDirSync(hermesCodingDir);
      fs.chmodSync(hermesCodingDir, 0o444);

      registerStateCommands(program, testDir);
      await program.parseAsync(['node', 'test', 'state', 'set', '--phase', 'clarify']);

      // Restore permissions for cleanup
      fs.chmodSync(hermesCodingDir, 0o755);
  ...
  describe('state clear', () => {
    ...
    it('should handle file system error during clear with JSON output', async () => {
      ...
      fs.chmodSync(hermesCodingDir, 0o555);
  ...
  describe('state archive', () => {
    ...
    it('should archive complete session successfully', async () => {
      ...
      fs.writeFileSync(path.join(hermesCodingDir, 'prd.md'), '# PRD\nTest content');
      fs.ensureDirSync(path.join(hermesCodingDir, 'tasks'));
      fs.writeFileSync(path.join(hermesCodingDir, 'tasks', 'index.json'), '{}');
      ...
      const archiveDir = path.join(hermesCodingDir, 'archive');
    ...
    it('should handle no session data to archive', async () => {
      ...
      // Ensure .hermes-coding directory is empty
    ...
    it('should archive all session files including logs', async () => {
      ...
      fs.ensureDirSync(path.join(hermesCodingDir, 'tasks'));
      fs.writeFileSync(path.join(hermesCodingDir, 'tasks', 'index.json'), '{}');
      fs.writeFileSync(path.join(hermesCodingDir, 'progress.txt'), 'progress');
      fs.writeFileSync(path.join(hermesCodingDir, 'debug.log'), 'debug');
      ...
      expect(fs.existsSync(stateFile)).toBe(false);
      expect(fs.existsSync(path.join(hermesCodingDir, 'prd.md'))).toBe(false);
      expect(fs.existsSync(path.join(hermesCodingDir, 'tasks'))).toBe(false);
      expect(fs.existsSync(path.join(hermesCodingDir, 'progress.txt'))).toBe(false);
      expect(fs.existsSync(path.join(hermesCodingDir, 'debug.log'))).toBe(false);
    });

...
describe('Refactored tasks commands (integration with real service)', () => {
  ...
  describe('AC1 & AC4: Thin command handlers using service factory', () => {
    ...
    it('should NOT register batch as a task subcommand', () => {
      registerTaskCommands(program, testDir);

      const tasksCommand = program.commands.find(cmd => cmd.name() === 'tasks');
      const commandNames = tasksCommand?.commands.map(c => c.name()) || [];
      expect(commandNames).not.toContain('batch');
    });

    it('should reject tasks batch invocation as unknown command', async () => {
      registerTaskCommands(program, testDir);

      await program.parseAsync(['node', 'test', 'tasks', 'batch', '--operations', '[]']);

      // Commander calls process.exit for unknown subcommands
      expect(processExitSpy).toHaveBeenCalled();
      expect(processExitSpy).not.toHaveBeenCalledWith(0);
    });
  });

  describe('Removed circuit-breaker command regression', () => {
    it('should NOT register cb as a top-level command', async () => {
      ...
      expect(topLevelNames).not.toContain('circuit-breaker');
      expect(topLevelNames).not.toContain('cb');
    });

    it('should reject cb status invocation as unknown command', async () => {
      const { registerStateCommands } = await import('../../src/commands/state');
      const { registerStatusCommand } = await import('../../src/commands/status');
      const { registerDetectCommand } = await import('../../src/commands/detect');
      const { registerLoopCommand } = await import('../../src/commands/loop');
      ...
      await program.parseAsync(['node', 'test', 'cb', 'status']);

      // Commander calls process.exit for unknown top-level commands
      expect(processExitSpy).toHaveBeenCalled();
      expect(processExitSpy).not.toHaveBeenCalledWith(0);
    });
  });
  ...
  describe('AC2 & AC3: Commands delegate to service (no file operations)', () => {
    ...
    it('complete command should work via service', async () => {
      registerTaskCommands(program, testDir);
      ...
      await program.parseAsync(['node', 'test', 'tasks', 'complete', 'task1', '-d', '30m']);
  ...
  describe('tasks next - refactored to use service', () => {
    ...
    it('should handle no pending tasks via service', async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining('All tasks resolved')
      );
    });
    ...
    it('should return next task in JSON format', async () => {
      ...
      expect(parsed.data?.task?.id).toBe('next.task');
    });
  });

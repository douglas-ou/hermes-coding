...
describe('MockFileSystem', () => {
  ...
  describe('remove', () => {
    ...
    it('should allow tests to simulate remove failures', async () => {
      await mockFs.writeFile('/tmp/file.txt', 'content');
      mockFs.setRemoveHook((filePath) => {
        if (filePath === '/tmp/file.txt') {
          throw new Error('remove failed');
        }
      });

      await expect(mockFs.remove('/tmp/file.txt')).rejects.toThrow('remove failed');
      expect(mockFs.getFile('/tmp/file.txt')).toBe('content');
    });
  });
  ...
  describe('helper methods', () => {
    describe('renameNoClobber', () => {
      it('should move a file when the destination does not exist', async () => {
        await mockFs.writeFile('/tmp/file.txt', 'content');

        await mockFs.renameNoClobber('/tmp/file.txt', '/dest/file.txt');

        expect(mockFs.hasFile('/tmp/file.txt')).toBe(false);
        expect(mockFs.getFile('/dest/file.txt')).toBe('content');
      });

      it('should throw EEXIST when the destination already exists', async () => {
        await mockFs.writeFile('/tmp/file.txt', 'content');
        await mockFs.writeFile('/dest/file.txt', 'existing');

        await expect(
          mockFs.renameNoClobber('/tmp/file.txt', '/dest/file.txt')
        ).rejects.toThrow('EEXIST');

        expect(mockFs.getFile('/tmp/file.txt')).toBe('content');
        expect(mockFs.getFile('/dest/file.txt')).toBe('existing');
      });
    });

    describe('writeFileNoClobber', () => {
      it('should write a file when the destination does not exist', async () => {
        await mockFs.writeFileNoClobber('/tmp/new.txt', 'content');

        expect(mockFs.getFile('/tmp/new.txt')).toBe('content');
      });

      it('should throw EEXIST without overwriting when the destination already exists', async () => {
        await mockFs.writeFile('/tmp/new.txt', 'existing');

        await expect(
          mockFs.writeFileNoClobber('/tmp/new.txt', 'replacement')
        ).rejects.toThrow('EEXIST');

        expect(mockFs.getFile('/tmp/new.txt')).toBe('existing');
      });
    });

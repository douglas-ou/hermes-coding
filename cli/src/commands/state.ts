...
export function registerStateCommands(program: Command, workspaceDir: string): void {
  const state = program.command('state').description('Manage hermes-coding state');

  // Get current state
  state
    .command('get')
    .description('Get current hermes-coding state')
      ...
      try {
        ...
        if (!currentState) {
          ...
          outputResponse(response, options.json, () => {
            console.log(chalk.yellow('No active hermes-coding session'));
          });
  ...
  state
    .command('set')
    .description('Set hermes-coding state')
    .option('-p, --phase <phase>', 'Current phase')
    ...
    .action(async (options) => {
      try {
        ...
        // Validate phase
        const validPhases: Phase[] = ['clarify', 'breakdown', 'implement', 'deliver', 'complete'];
        ...
        if (existingState) {
          ...
          newState = await stateService.updateState(updates);
        } else {
          if (!options.phase) {
            handleError(Errors.invalidInput('Phase is required when initializing state'), options.json);
            return;
          }
  ...
  state
    .command('clear')
    .description('Clear hermes-coding state')
    .option('--json', 'Output as JSON')
    .action(async (options) => {
      try {
        const stateService = createStateService(workspaceDir);
  ...
  state
    .command('archive')
    .description('Archive current session to .hermes-coding/archive/ and clear state')

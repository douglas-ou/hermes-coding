...
export const Errors = {
  taskNotFound: (taskId: string): CLIError => createError(
    ...
    {
      recoverable: false,
      suggestedAction: 'Use "hermes-coding tasks list" to see available tasks',
    }
  ),

  stateNotFound: (): CLIError => createError(
    'STATE_NOT_FOUND',
    'No active hermes-coding session',
    {
      recoverable: false,
      suggestedAction: 'Start a new session with /hermes-coding command',
    }
  ),

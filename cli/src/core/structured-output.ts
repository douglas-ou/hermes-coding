...
export const implementationResultSchema = z.object({
  ...
  // Cross-task learnings (persisted to progress.txt for future tasks)
  learnings: z.array(z.object({
    type: z.enum(['pattern', 'gotcha', 'context']),
    content: z.string(),
    scope: z.enum(['global', 'directory']),
    directory: z.string().optional(),
  })).optional().describe('Reusable learnings for future tasks'),
});
...
export const healingResultSchema = z.object({
  ...
  // Reusable fix pattern for future healing (persisted to progress.txt)
  heal_pattern: z.object({
    error_signature: z.string(),
    fix_command: z.string(),
  }).optional().describe('Reusable error-to-fix mapping for future healing'),
});
        ...
        learnings: {
          ...
          items: {
            ...
            properties: {
              type: {
                type: 'string',
                enum: ['pattern', 'gotcha', 'context'],
                description: 'Learning category: pattern (reusable approach), gotcha (pitfall), context (useful location/info)'
              },
              content: { type: 'string', description: 'The learning itself' },
              scope: {
                type: 'string',
                enum: ['global', 'directory'],
                description: 'Whether this applies project-wide or to a specific directory'
              },
              directory: {
                type: 'string',
                description: 'Directory path (only when scope=directory)'
              },
            },
            required: ['type', 'content', 'scope'],
          },
          description: 'Reusable learnings for future tasks'
        },
      },
      ...
      properties: {
        ...
        heal_pattern: {
          ...
          properties: {
            error_signature: {
              type: 'string',
              description: 'Error message or pattern that identifies this failure (e.g., "Module not found: @prisma/client")'
            },
            fix_command: {
              type: 'string',
              description: 'Command or action that fixes this error (e.g., "npx prisma generate")'
            },
          },
          required: ['error_signature', 'fix_command'],
          description: 'Reusable error-to-fix mapping for future healing'
        },
      },

...
const CACHE_FILE = 'hermes-coding-update-check.json';

/**
 * Get cache directory path
 * Uses ~/.config/configstore/ (standard location for CLI tools)
 * Supports HERMES_CODING_CACHE_DIR environment variable for testing
 */
function getCacheDir(): string {
  return process.env.HERMES_CODING_CACHE_DIR || join(homedir(), '.config', 'configstore');
}
...
export function showUpdateNotification(
  ...
  const message = [
    ...
    chalk.yellow('│') +
      ...
      chalk.yellow('│'),
    chalk.yellow('│') +
      chalk.cyan(`   Run: hermes-coding update`) +
      ' '.repeat(35) +
      chalk.yellow('│'),
    chalk.yellow('│') +

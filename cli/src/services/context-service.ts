...
export class ContextService implements IContextService {
  ...
  private async getRecentActivity(): Promise<string[]> {
    const progressTxt = path.join(this.workspaceDir, '.hermes-coding', 'progress.txt');

    try {
      const exists = await this.fileSystem.exists(progressTxt);
      if (!exists) {
        return [];
      }

      const progressRaw = await this.fileSystem.readFile(progressTxt, 'utf-8');
      const content = typeof progressRaw === 'string' ? progressRaw : progressRaw.toString('utf-8');
      const lines = content.trim().split('\n');
      return lines.slice(-5);
    } catch {
      return ['Unable to read progress.txt'];
    }
  }

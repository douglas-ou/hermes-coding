...
export class MockFileSystem implements IFileSystem {
  ...
  private renameNoClobberHook?: (oldPath: string, newPath: string) => Promise<void> | void;
  private removeHook?: (filePath: string) => Promise<void> | void;
   ...
   */
  async writeFileNoClobber(filePath: string, data: string | Buffer, options?: WriteFileOptions): Promise<void> {
    const normalizedPath = this.normalizePath(filePath);

    if (this.files.has(normalizedPath)) {
      const error: any = new Error(`EEXIST: file already exists, open '${filePath}'`);
      error.code = 'EEXIST';
      throw error;
    }
  ...
  async remove(filePath: string): Promise<void> {
    ...
    if (this.removeHook) {
      await this.removeHook(normalizedPath);
    }
  ...
  /**
   * Rename a file in memory (move from oldPath to newPath)
   */
  async rename(oldPath: string, newPath: string): Promise<void> {
    ...
    if (!this.files.has(normalizedOld)) {
      const error: any = new Error(`ENOENT: no such file or directory, rename '${oldPath}'`);
      error.code = 'ENOENT';
      throw error;
    }

    const content = this.files.get(normalizedOld)!;
    await this.ensureDir(path.dirname(normalizedNew));
  ...
  async renameNoClobber(oldPath: string, newPath: string): Promise<void> {
    ...
    if (this.renameNoClobberHook) {
      await this.renameNoClobberHook(normalizedOld, normalizedNew);
    }

    if (this.files.has(normalizedNew)) {
      const error: any = new Error(`EEXIST: file already exists, rename '${newPath}'`);
      error.code = 'EEXIST';
      throw error;
    }
  ...
  /**
   * Inject a hook that runs immediately before renameNoClobber checks
   * the destination path. Useful for race-condition tests.
   */
  setRenameNoClobberHook(
    hook?: (oldPath: string, newPath: string) => Promise<void> | void
  ): void {
    this.renameNoClobberHook = hook;
  }

  /**
   * Inject a hook that runs before remove mutates the mock filesystem.
   * Useful for rollback-failure tests.
   */
  setRemoveHook(
    hook?: (filePath: string) => Promise<void> | void
  ): void {
